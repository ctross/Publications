...
fitParasitesViolence<- stan(model_code=model_code, data = model_dat,inits=0, thin=10,iter = 20000, warmup=2000,chains = 1)
 
  print(fitParasitesViolence,digits_summary=3)
  traceplot(fitParasitesViolence,ask=T,pars=c("Beta"))
  
  Beta<-extract(fitParasitesViolence, pars="Beta")$Beta
...
############ Plot Results
library(Cairo)
CairoPDF(width=8, height=8, "ViolenceModel.pdf")

  plot(mPred~Theta,ylim=c(0,1),type="n",col=rgb(1, 0, 0,alpha=.9),ylab="Probability", xlab="Parasite Prevalence" )
