...
fitParasitesCollectivism<- stan(model_code=model_code, data = model_dat,inits=0, iter = 4000, warmup=2000,chains = 1)

  print(fitParasitesCollectivism,digits_summary=3)
  
  Beta<-extract(fitParasitesCollectivism, pars="Beta")$Beta
  C<-extract(fitParasitesCollectivism, pars="C")$C
...
############ Plot Results
library(Cairo)
CairoPDF(width=8, height=8, "CollectivismModel.pdf")
  ...
  for(z in 1:200){
  zz<-z*10
  lines(Pred[zz,i+1,]~Theta, col=rgb((i*.1), 0, (1-(i*.1)),alpha=.05))}
  }
