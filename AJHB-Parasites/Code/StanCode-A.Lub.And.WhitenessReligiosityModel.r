...
########################################################################################################## STAN MODEL Data 
model_dat  <-list(S=S,L=L,D=D,K=K,TotalW=TotalW, Whites=Whites,Parasites=d$sumPositive, Tested=d$sumTested,Outcome=Y)
...
for(l in 1:L){
  Whites[l]~binomial(TotalW[l],inv_logit(ThetaW[l]));
}
...
fitParasitesReligion<- stan(model_code=model_code, data = model_dat,inits=0, iter = 4000, warmup=2000,chains = 1)

   print(fitParasitesReligion,digits_summary=3)
  traceplot(fitParasitesReligion,ask=T,pars=c("C","Beta"))
  
  Beta<-extract(fitParasitesReligion, pars="Beta")$Beta
  C<-extract(fitParasitesReligion, pars="C")$C
...
############ Plot Results
library(Cairo)
CairoPDF(width=8, height=8, "ReligiosityModel.pdf")
