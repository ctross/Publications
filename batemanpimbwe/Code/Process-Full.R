...
##################################################################### Plot Densities
 ...
 aes(x=x,y=y), fill = '#e31a1c',alpha=0.7)   + ylab("Density")+xlab("Elasticity: Spouse Number")+
 ...
 legend.text=element_text(size=22), legend.title=element_text(size=22,face="bold"))

 ggsave("ElastNS.pdf",p0,height=8,width=8)
...
##################################################################### Plot Densities
 ...
 aes(x=x,y=y), fill = '#e31a1c',alpha=0.7)   + ylab("Density")+xlab("Elasticity: Weighted Spouse Years")+
 ...
 legend.text=element_text(size=22), legend.title=element_text(size=24,face="bold"))
  ggsave("ElastSY.pdf",p1,height=8,width=8)
...
p2 <- ggplot(plot1,aes(x=Age,y=Res1Mm)) +  ylab("Relative Value of Marriage")+ xlab("Age")+
...
p3 <- ggplot(plot1,aes(x=Age,y=Res1M1f)) +  ylab("Relative Value of Marriage")+
      geom_ribbon(aes(ymin=Res1M1f,ymax=Res1M2f),alpha=0.5,fill="#e31a1c")+  xlab("Age")+
      geom_line(aes(x=Age,y=Res1Mf),color="#e31a1c",size=2)+
      theme(axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"))

#CairoPDF("SelfM",height=8,width=8)
  p2
# dev.off()
#CairoPDF("SelfF",height=8,width=8)
  p3
# dev.off()
...
library(rethinking)
MaxExposure<-86
SpouseAgeEffect_males<-extract(fitDem,pars="SpouseAgeEffect_males")$SpouseAgeEffect_males
Phi_males<-extract(fitDem,pars="Phi_males")$Phi_males
  ...
  for (a in 1:Age_males[i]){
   if(PartnersByAge1_males[i,a+11]==0){ESA[1] <-0;}
         ...
         for(b in 1:MaxExposure){
          Scrap[b]<-MissingDataWeights_males[b]*inv_logit(Phi_males[k,2] + SpouseAgeEffect_males[k,b]);}
          ESA[1] <- sum(Scrap);}
    else{ ESA[1] <-inv_logit(Phi_males[k,2] + SpouseAgeEffect_males[k,PartnersByAge1_males[i,a+11]]);}}
   ...
   if(PartnersByAge2_males[i,a+11]==0){ESA[2] <-0;}
         ...
         for(b in 1:MaxExposure){
          Scrap[b]<-MissingDataWeights_males[b]*inv_logit(Phi_males[k,2] + SpouseAgeEffect_males[k,b]);}
          ESA[2] <- sum(Scrap);}
    else{ ESA[2] <-inv_logit(Phi_males[k,2] + SpouseAgeEffect_males[k,PartnersByAge2_males[i,a+11]]);}}
   ...
   if(PartnersByAge3_males[i,a+11]==0){ESA[3] <-0;}
         ...
         for(b in 1:MaxExposure){
          Scrap[b]<-MissingDataWeights_males[b]*inv_logit(Phi_males[k,2] + SpouseAgeEffect_males[k,b]);}
          ESA[3] <- sum(Scrap);}
    else{ ESA[3] <-inv_logit(Phi_males[k,2] + SpouseAgeEffect_males[k,PartnersByAge3_males[i,a+11]]);}}
   ...
   if(PartnersByAge4_males[i,a+11]==0){ESA[4] <-0;}
         ...
         for(b in 1:MaxExposure){
          Scrap[b]<-MissingDataWeights_males[b]*inv_logit(Phi_males[k,2] + SpouseAgeEffect_males[k,b]);}
          ESA[4] <- sum(Scrap);}
    else{ ESA[4] <-inv_logit(Phi_males[k,2] + SpouseAgeEffect_males[k,PartnersByAge4_males[i,a+11]]);}}
   ...
   if(PartnersByAge5_males[i,a+11]==0){ESA[5] <-0;}
         ...
         for(b in 1:MaxExposure){
          Scrap[b]<-MissingDataWeights_males[b]*inv_logit(Phi_males[k,2] + SpouseAgeEffect_males[k,b]);}
          ESA[5] <- sum(Scrap);}
    else{ ESA[5] <-inv_logit(Phi_males[k,2] + SpouseAgeEffect_males[k,PartnersByAge5_males[i,a+11]]);}}
...
################################################################################################################################
SpouseAgeEffect_females<-extract(fitDem,pars="SpouseAgeEffect_females")$SpouseAgeEffect_females
Phi_females<-extract(fitDem,pars="Phi_females")$Phi_females
 ...
 for (i in 1:N_females){
  for (a in 1:Age_females[i]){
   if(PartnersByAge1_females[i,a+11]==0){EffectiveSpousesByAge_females[i,a,k] <-0;}
         ...
         for(b in 1:MaxExposure){
         Scrap[b]<-MissingDataWeights_females[b]*inv_logit(Phi_females[k,2] + SpouseAgeEffect_females[k,b]);}
         EffectiveSpousesByAge_females[i,a,k] <- sum(Scrap);}
     else{EffectiveSpousesByAge_females[i,a,k] <-inv_logit(Phi_females[k,2] + SpouseAgeEffect_females[k,PartnersByAge1_females[i,a+11]]);}}
     }}
   }
...
SQ_PQ_SpouseYears_males<-array(NA,c(N_males,Samples))
PQ_SpouseYears_males<-array(NA,c(N_males,Samples))
...
for(k in 1:Samples){
 for (i in 1:N_males){
   ...
   Ticker4 <- 0;
 for (a in 1:Age_males[i]){
   Ticker  <- Ticker + inv_logit(Delta_males[k,2]  + MarriageAgeEffect_males[k,a])*EffectiveSpousesByAge_males[i,a,k];
   ...
   SQ_PQ_SpouseYears_males[i,k] <- Ticker;
   PQ_SpouseYears_males[i,k] <- Ticker2;
...
for(i in 1:N_males){
SQ_PQ_SpouseYears_males_median[i]<-median(SQ_PQ_SpouseYears_males[i,])
PQ_SpouseYears_males_median[i]<-median(PQ_SpouseYears_males[i,])
...
SQ_PQ_SpouseYears_females<-array(NA,c(N_females,Samples))
PQ_SpouseYears_females<-array(NA,c(N_females,Samples))
...
for(k in 1:Samples){
 for (i in 1:N_females){
   ...
   Ticker4 <- 0;
 for (a in 1:Age_females[i]){
   Ticker <- Ticker + inv_logit(Delta_females[k,2]  + MarriageAgeEffect_females[k,a])*EffectiveSpousesByAge_females[i,a,k];
   ...
   SQ_PQ_SpouseYears_females[i,k] <- Ticker;
   PQ_SpouseYears_females[i,k] <- Ticker2;
...
for(i in 1:N_females){
SQ_PQ_SpouseYears_females_median[i]<-median(SQ_PQ_SpouseYears_females[i,])
PQ_SpouseYears_females_median[i]<-median(PQ_SpouseYears_females[i,])
...
cv<-function(x){var(x)/(mean(x)^2)}
 ...
 Ims.f<-c()
      for(i in 1:Samples){
 Ims.m[i]<- cv(sample(SQ_PQ_SpouseYears_males[which(Age_males >45),i], replace = TRUE))
 Ims.f[i]<- cv(sample(SQ_PQ_SpouseYears_females[which(Age_females >45),i], replace = TRUE))
                              }
 ...
 Mms.f<-c()
   for(i in 1:Samples){
     xm1 <- sample(c(1:length(RS_males)), replace = TRUE)
     xf1 <- sample(c(1:length(RS_females)), replace = TRUE)
   Mms.m[i]<- (M_index(SQ_PQ_SpouseYears_males[xm1,i],Age_males[xm1])/sqrt(mean(SQ_PQ_SpouseYears_males[xm1,i])))^2
   Mms.f[i]<- (M_index(SQ_PQ_SpouseYears_females[xf1,i],Age_females[xf1])/sqrt(mean(SQ_PQ_SpouseYears_females[xf1,i])))^2
                              }
...
for(k in 1:Samples){
 for (i in 1:N_males){
  if(Zeros_males[i]==0){
   ...
   A_males[i,k] <- exp(Beta_males[k,1] + Beta_males[k,2]*log(Age_males[i])  + Beta_males[k,3]*log(NumberSpouses_males[i]) + exp(Beta_males[k,4])*log(SQ_PQ_SpouseYears_males[i,k])); # + SecularEffect[YOB_scaled_males[i]]
   } else{
   B_males[i,k]<- -1
   A_males[i,k] <- -1
   }
...
for(k in 1:Samples){
 ...
 if(Zeros_females[i]==0){
   ...
   A_females[i,k] <- exp(Beta_females[k,1] + Beta_females[k,2]*log(Age_females[i]) + Beta_females[k,3]*log(NumberSpouses_females[i]) + exp(Beta_females[k,4])*log(SQ_PQ_SpouseYears_females[i,k])); # + SecularEffect[YOB_scaled_females[i]]
    } else{
   B_females[i,k] <- -1
   A_females[i,k] <- -1
   }
...
write.csv(ResTab,"Bateman-FM.csv")
