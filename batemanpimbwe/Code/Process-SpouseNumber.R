...
cv<-function(x){var(x)/(mean(x)^2)}
 ...
 Ims.f<-c()
      for(i in 1:Samples){
 Ims.m[i]<- cv(sample(NumberSpouses_males[which(Age_males >45)], replace = TRUE))
 Ims.f[i]<- cv(sample(NumberSpouses_females[which(Age_females >45)], replace = TRUE))
                              }
...
for(k in 1:Samples){
 for (i in 1:N_males){
  if(Zeros_males[i]==0){
   ...
   A_males[i,k] <- exp(Beta_males[k,1] + Beta_males[k,2]*log(Age_males[i]) + Beta_males[k,3]*log(NumberSpouses_males[i])); # + SecularEffect[YOB_scaled_males[i]]
   } else{
   B_males[i,k]<- -1
   A_males[i,k] <- -1
   }
...
for(k in 1:Samples){
 ...
 if(Zeros_females[i]==0){
   ...
   A_females[i,k] <- exp(Beta_females[k,1] + Beta_females[k,2]*log(Age_females[i]) + Beta_females[k,3]*log(NumberSpouses_females[i])); # + SecularEffect[YOB_scaled_females[i]]
    } else{
   B_females[i,k] <- -1
   A_females[i,k] <- -1
   }
