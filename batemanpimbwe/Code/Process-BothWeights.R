...
for(k in 1:Samples){
 for (i in 1:N_males){
  if(Zeros_males[i]==0){
   ...
   A_males[i,k] <- exp(Beta_males[k,1] + Beta_males[k,2]*log(Age_males[i]) + exp(Beta_males[k,4])*log(SQ_PQ_SpouseYears_males[i,k])); # + SecularEffect[YOB_scaled_males[i]]
   } else{
   B_males[i,k]<- -1
   A_males[i,k] <- -1
   }
...
for(k in 1:Samples){
 ...
 if(Zeros_females[i]==0){
   ...
   A_females[i,k] <- exp(Beta_females[k,1] + Beta_females[k,2]*log(Age_females[i]) + exp(Beta_females[k,4])*log(SQ_PQ_SpouseYears_females[i,k])); # + SecularEffect[YOB_scaled_females[i]]
    } else{
   B_females[i,k] <- -1
   A_females[i,k] <- -1
   }
...
write.csv(ResTab,"Bateman-TSQWSY.csv")
