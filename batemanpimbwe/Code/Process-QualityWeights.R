...
p4 <- ggplot(plot1,aes(x=Age,y=Res1M1m)) +  ylab("Relative Value of Spouse to RS of Ego")+ xlab("Spouse Age")+ ylim(c(NA, 1))+
      geom_ribbon(aes(ymin=Res1M1m,ymax=Res1M2m),alpha=0.5,fill="#253494")+
      geom_line(aes(x=Age,y=Res1Mm),color="#253494",size=2)+
      theme(axis.text=element_text(size=20), axis.title=element_text(size=22,face="bold"))

p5 <- ggplot(plot1,aes(x=Age,y=Res1M1f)) +  ylab("Relative Value of Spouse to RS of Ego")+
      geom_ribbon(aes(ymin=Res1M1f,ymax=Res1M2f),alpha=0.5,fill="#e31a1c")+  xlab("Spouse Age")+  ylim(c(NA, 1))+
      geom_line(aes(x=Age,y=Res1Mf),color="#e31a1c",size=2)+
      theme(axis.text=element_text(size=20), axis.title=element_text(size=22,face="bold"))

 ggsave("SpouseM.pdf",p4,height=8,width=8)

 ggsave("SpouseF.pdf",p5,height=8,width=8)
...
cv<-function(x){var(x)/(mean(x)^2)}
 ...
 Ims.f<-c()
      for(i in 1:Samples){
 Ims.m[i]<- cv(sample(PQ_SpouseYears_males[which(Age_males >45),i], replace = TRUE))
 Ims.f[i]<- cv(sample(PQ_SpouseYears_females[which(Age_females >45),i], replace = TRUE))
                              }
 ...
 Mms.f<-c()
   for(i in 1:Samples){
     xm1 <- sample(c(1:length(RS_males)), replace = TRUE)
     xf1 <- sample(c(1:length(RS_females)), replace = TRUE)
   Mms.m[i]<- (M_index(PQ_SpouseYears_males[xm1,i],Age_males[xm1])/sqrt(mean(PQ_SpouseYears_males[xm1,i])))^2
   Mms.f[i]<- (M_index(PQ_SpouseYears_females[xf1,i],Age_females[xf1])/sqrt(mean(PQ_SpouseYears_females[xf1,i])))^2
                              }
...
for(k in 1:Samples){
 for (i in 1:N_males){
  if(Zeros_males[i]==0){
   ...
   A_males[i,k] <- exp(Beta_males[k,1] + Beta_males[k,2]*log(Age_males[i]) + exp(Beta_males[k,4])*log(PQ_SpouseYears_males[i,k]));
   } else{
   B_males[i,k]<- -1
   A_males[i,k] <- -1
   }
...
for(k in 1:Samples){
 ...
 if(Zeros_females[i]==0){
   ...
   A_females[i,k] <- exp(Beta_females[k,1] + Beta_females[k,2]*log(Age_females[i]) + exp(Beta_females[k,4])*log(PQ_SpouseYears_females[i,k]));
    } else{
   B_females[i,k] <- -1
   A_females[i,k] <- -1
   }
...
write.csv(ResTab,"Bateman-SQWSY.csv")
